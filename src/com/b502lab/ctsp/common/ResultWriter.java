...
public class ResultWriter {
	public static void write(String filePath, Problem problem, List<Integer> bestTour, double bestFitness,
		...
		try (FileWriter out = new FileWriter(filePath)) {
			...
			for (int i = 0; i < x.length; i++) {
				x[i] = problem.CITY_INFO[bestTour.get(i)].x;
				y[i] = problem.CITY_INFO[bestTour.get(i)].y;
			}
			out.write("\r\n对应的访问坐标为 ->\r\n" + Arrays.toString(x) + "\r\n" + Arrays.toString(y));
