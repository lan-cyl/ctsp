...
public class LocalSearch {
     ...
     */
    public static void two_opt_first(int[] tour) {
...
//        boolean[] dlb; /* vector containing don't look bits */
        ...
        pos = new int[Tsp.me.n];
        for (i = 0; i < Tsp.me.n; i++) {
            pos[tour[i]] = i;
        }

        improvement_flag = true;
        random_vector = generate_random_permutation(Tsp.me.n);

        while (improvement_flag) {

            improvement_flag = false;

            for (l = 0; l < Tsp.me.n; l++) {
		...
		/* First search for c1's nearest neighbours, use successor of c1 */
            ...
            /* Search one for next c1's h-nearest neighbours, use predecessor c1 */
                    if (pos_c1 > 0)
                        ...
                        p_c1 = tour[Tsp.me.n - 1];
                    radius = Tsp.me.distance[p_c1][c1];
                    for (h = 0; h < Tsp.me.nn_ls; h++) {
                        c2 = Tsp.me.nn_list[c1][h]; /* exchange partner, determine its position */
                        if (radius > Tsp.me.distance[c1][c2]) {
                            pos_c2 = pos[c2];
                            if (pos_c2 > 0)
                                ...
                                p_c2 = tour[Tsp.me.n - 1];
                            if (p_c2 == c1)
                                continue;
                            if (p_c1 == c2)
                                continue;
                            gain = -radius + Tsp.me.distance[c1][c2] + Tsp.me.distance[p_c1][p_c2]
                                    - Tsp.me.distance[p_c2][c2];
                            if (gain < 0) {
                                h1 = p_c1;
                                h2 = c1;
                                h3 = p_c2;
    ...
    private static void exchange(int[] tour, int[] pos, int h1, int h2, int h3, int h4) {
        ...
        if (pos[h3] - pos[h2] < Tsp.me.n / 2 + 1) {
            ...
            tour[Tsp.me.n] = tour[0];
        }
    }

    public static void three_opt_first(int[] tour)
	 ...
	 */
        ...
        pos = new int[Tsp.me.n];
        h_tour = new int[Tsp.me.n];
        hh_tour = new int[Tsp.me.n];
        ...
        while (improvement_flag) {
            ...
            for (l = 0; l < Tsp.me.n; l++) {
                ...
                whileLoop:
                while (h < Tsp.me.nn_ls) {

                    c2 = Tsp.me.nn_list[c1][h]; /* second city, determine its position */
                    pos_c2 = pos[c2];
                    s_c2 = tour[pos_c2 + 1];
                    if (pos_c2 > 0)
                        p_c2 = tour[pos_c2 - 1];
                    ...
                    radius = Tsp.me.distance[c1][s_c1];
                    add1 = Tsp.me.distance[c1][c2];

		    /* Here a fixed radius neighbour search is performed */
                    if (radius > add1) {
                        decrease_breaks = -radius - Tsp.me.distance[c2][s_c2];
                        diffs = decrease_breaks + add1 + Tsp.me.distance[s_c1][s_c2];
                        diffp = -radius - Tsp.me.distance[c2][p_c2] + Tsp.me.distance[c1][p_c2]
                                + Tsp.me.distance[s_c1][c2];
                    } else
                        break;
            ...
            /* Now perform the innermost search */
                    g = 0;
                    while (g < Tsp.me.nn_ls) {

                        c3 = Tsp.me.nn_list[s_c1][g];
                        pos_c3 = pos[c3];
                        s_c3 = tour[pos_c3 + 1];
                        if (pos_c3 > 0)
                            p_c3 = tour[pos_c3 - 1];
                        else
                            p_c3 = tour[Tsp.me.n - 1];

                        if (c3 == c1) {
                            g++;
                            continue;
                        } else {
                            add2 = Tsp.me.distance[s_c1][c3];
                /* Perform fixed radius neighbour search for innermost search */
				     ...
				     */
                                    ...
                                    gain = decrease_breaks - Tsp.me.distance[c3][p_c3] + add1 + add2
                                            + Tsp.me.distance[p_c3][s_c2];
				    ...
				    /* We have to add edges (c1,c2), (s_c1,c3), (s_c2,s_c3) */
                                    ...
                                    gain = decrease_breaks - Tsp.me.distance[c3][s_c3] + add1 + add2
                                            + Tsp.me.distance[s_c2][s_c3];
				    ...
				    /* or add edges (c1,c2), (s_c1,c3), (p_c2,p_c3) */
                                    gain = -radius - Tsp.me.distance[p_c2][c2] - Tsp.me.distance[p_c3][c3]
                                            + add1 + add2 + Tsp.me.distance[p_c2][p_c3];
				     ...
				     */
                                    ...
                                    gain = -radius - Tsp.me.distance[p_c2][c2] - Tsp.me.distance[c3][s_c3]
                                            + add1 + add2 + Tsp.me.distance[p_c2][s_c3];
		    ...
		    /* Now make the exchange */
                    if (move_flag != 0) {
                        ...
                        if (move_flag == 4) {
                            ...
                            if (pos_c2 > pos_c1)
                                ...
                                n1 = Tsp.me.n - (pos_c1 - pos_c2);
                            if (pos_c3 > pos_c2)
                                n2 = pos_c3 - pos_c2;
                            else
                                n2 = Tsp.me.n - (pos_c2 - pos_c3);
                            if (pos_c1 > pos_c3)
                                n3 = pos_c1 - pos_c3;
                            else
                                n3 = Tsp.me.n - (pos_c3 - pos_c1);
				...
				/* Now copy stored part from h_tour */
                            ...
                            if (pos_c3 < pos_c2)
                                ...
                                n1 = Tsp.me.n - (pos_c3 - pos_c2);
                            if (pos_c3 > pos_c1)
                                n2 = pos_c3 - pos_c1 + 1;
                            else
                                n2 = Tsp.me.n - (pos_c1 - pos_c3 + 1);
                            if (pos_c2 > pos_c1)
                                n3 = Tsp.me.n - (pos_c2 - pos_c1 + 1);
                            else
                                n3 = pos_c1 - pos_c2 + 1;
    ...
    private static int[] generate_random_permutation(int n) {
        ...
        /* find (randomly) an index for a free unit */
            node = (int) (RANDOM_GEN.nextDouble() * (n - tot_assigned));
            assert (i + node < n);
            help = r[i];
            r[i] = r[i + node];
            r[i + node] = help;
